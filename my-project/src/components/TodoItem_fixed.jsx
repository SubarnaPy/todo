...
const TodoItem = ({ task, onUpdateTask, onDeleteTask, onToggleTask }) => {
  ...
  return (
    <>
      <div
        className={`relative glass-card rounded-3xl p-6 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-2xl border-l-8 cursor-pointer ${
          status
        ...
        } ${isHovered ? 'shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50' : 'shadow-lg'} backdrop-blur-sm`}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className={`flex items-center space-x-2 transition-all duration-300 ${
                    isHovered || (typeof window !== 'undefined' && window.innerWidth < 768) ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'
                  }`}>
                    <button
                      onClick={handleEdit}
                      disabled={isLoading}
                      className="p-2.5 text-indigo-600 dark:text-indigo-400 hover:bg-gradient-to-br hover:from-indigo-100 hover:to-blue-100 dark:hover:from-indigo-900/30 dark:hover:to-blue-900/30 rounded-2xl disabled:opacity-50 transition-all duration-300 shadow-lg hover:shadow-indigo-200 dark:hover:shadow-indigo-900/50 transform hover:scale-110"
                      title="Edit task"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="p-2.5 text-red-500 dark:text-red-400 hover:bg-gradient-to-br hover:from-red-100 hover:to-pink-100 dark:hover:from-red-900/30 dark:hover:to-pink-900/30 rounded-2xl disabled:opacity-50 transition-all duration-300 shadow-lg hover:shadow-red-200 dark:hover:shadow-red-900/50 transform hover:scale-110"
                      title="Delete task"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm">
                  ...
                  {priority && priority !== 'medium' && (
                    <div className={`flex items-center px-3 py-1.5 rounded-2xl ${priorityConfig.bgColor} ${priorityConfig.textColor} border-2 ${priorityConfig.borderColor} shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105`}>
                  ...
                  {/* Category badge */}
                  {category && (
                    <div className="flex items-center px-3 py-1.5 rounded-2xl bg-gradient-to-r from-purple-100 via-pink-100 to-violet-100 dark:from-purple-900/30 dark:via-pink-900/30 dark:to-violet-900/30 text-purple-700 dark:text-purple-300 border-2 border-purple-200 dark:border-purple-700 shadow-lg hover:shadow-purple-200 dark:hover:shadow-purple-900/50 transition-all duration-300 transform hover:scale-105">
                  ...
                  {/* Status badge */}
                  <div className={`flex items-center px-3 py-1.5 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 ${
                    status
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-md w-full mx-4 overflow-hidden transform transition-all duration-300 scale-100">
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-6 text-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold">Task Actions</h3>
                    <p className="text-white/80 text-sm">Choose what to do with this task</p>
                  </div>
                </div>
                <button
                  onClick={closeModal}
                  className="p-2 hover:bg-white/20 rounded-2xl transition-all duration-200"
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  {priority && priority !== 'medium' && (
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${priorityConfig.bgColor} ${priorityConfig.textColor}`}>
                      {priorityConfig.icon} {priority.charAt(0).toUpperCase() + priority.slice(1)}
                    </span>
                  )}
                  {category && (
              ...
              <div className="space-y-3">
                {/* Mark as Complete */}
                <button
                  onClick={() => { setModalAction('complete'); handleModalAction(); }}
                  className="w-full p-4 bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 hover:from-emerald-600 hover:via-green-600 hover:to-teal-600 text-white rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-emerald-200 dark:hover:shadow-emerald-900/50 flex items-center justify-center space-x-3"
                >
                  ...
                  <span>{status ? 'Mark as Pending' : 'Mark as Complete'}</span>
                </button>

                {/* Edit Task */}
                <button
                  onClick={() => { setModalAction('edit'); handleModalAction(); }}
                  className="w-full p-4 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 text-white rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-indigo-200 dark:hover:shadow-indigo-900/50 flex items-center justify-center space-x-3"
                >
                  <div className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                ...
                {/* Delete Task */}
                <button
                  onClick={() => { setModalAction('delete'); handleModalAction(); }}
                  className="w-full p-4 bg-gradient-to-r from-red-500 via-pink-500 to-red-600 hover:from-red-600 hover:via-pink-600 hover:to-red-700 text-white rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-red-200 dark:hover:shadow-red-900/50 flex items-center justify-center space-x-3"
                >
                  <div className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              ...
              <button
                onClick={closeModal}
                className="w-full p-3 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105"
              >
                Cancel
              </button>
            </div>
